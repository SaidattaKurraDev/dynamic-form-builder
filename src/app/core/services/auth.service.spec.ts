...
describe('AuthService', () => {
  ...
  it('should call login and check if the api is working or not', () => {
    ...
    service.login('admin@formBuilder.com', 'admin123').subscribe(users => {
      expect(users.length).toBe(1);
      expect(users[0].email).toBe('admin@formBuilder.com');
    });
    const req = httpMock.expectOne(
      'http://localhost:3000/users?email=admin@formBuilder.com&password=admin123'
    )
  ...
  it('should get user', () => {
    const mockUser = { id: 1, email: 'admin@formBuilder.com', password: 'admin123', role: 'admin' };
    service.setUser(mockUser);
